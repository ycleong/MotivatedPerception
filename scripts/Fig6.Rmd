...
if (redo_calc) {
  ...
  res.prob.bias.coop = lmer(Prob ~ Cat_n_z  + Pred + (Pred |Sub), subset(BiasedData, Con == "Coop"))
  res.prob.bias.comp = lmer(Prob ~ Cat_n_z  + Pred + (Pred |Sub), subset(BiasedData, Con == "Comp"))
  
  res.prob.unbias = lmer(Prob ~ Cat_n_z  + Con * Pred + (Con * Pred |Sub), UnbiasedData)
  res.prob.unbias.coop = lmer(Prob ~ Cat_n_z  + Pred + (Pred |Sub), subset(UnbiasedData, Con == "Coop"))
  res.prob.unbias.comp = lmer(Prob ~ Cat_n_z  + Pred + (Pred |Sub), subset(UnbiasedData, Con == "Comp"))

  
  save(res.prob, res.prob.bias, res.prob.bias.coop, res.prob.bias.comp, res.prob.unbias, res.prob.unbias.coop, res.prob.unbias.comp, file="long_calc/NeuralBias.Rda")
} else {
...
lm.neuralbias = summary(lm(scale(NeuralBias) ~ scale(drift_bias) + scale(z), data = full.model.subj_parm))
lm.neuralbias
lm.neuralbias.coef = lm.neuralbias$coefficients

reg_coef = NULL
reg_coef$parm = c('z_bias','v_bias')
reg_coef$value = c(lm.neuralbias.coef["scale(z)","Estimate"],lm.neuralbias.coef["scale(drift_bias)","Estimate"])
reg_coef$SE = c(lm.neuralbias.coef["scale(z)","Std. Error"],lm.neuralbias.coef["scale(drift_bias)","Std. Error"])
...
plot.reg_coef = ggplot(reg_coef) +
  ...
  ggtitle("Neural Bias ~ drift_bias + z_bias + e")

# Plot all
plot_grid(plot.prob.bias,plot.prob.unbias,plot.prob.behav, plot.reg_coef, labels = c('A','','B', 'C'), ncol = 2)
```
